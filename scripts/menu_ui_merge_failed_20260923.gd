...
var settings_content_container: VBoxContainer = null
var ship_change_pending: bool = false
var ship_change_target_id: String = ""
var ship_change_elapsed: float = 0.0
var ship_change_total_seconds: float = 5.0
...
func _build_menu_button() -> void:
				...
				var menu_top := 172.0 if not is_mobile else 218.0
				var menu_bottom := 216.0 if not is_mobile else 262.0
				var config_top := 220.0 if not is_mobile else 266.0
				var config_bottom := 250.0 if not is_mobile else 300.0
				...
				menu_button.offset_bottom = menu_bottom
				...
				config_indicator.offset_top = config_top
				config_indicator.offset_right = -20.0
				config_indicator.offset_bottom = config_bottom
...
func _build_main_menu() -> Control:
				...
				for i in range(items.size()):
								...
								if items[i] == "EKİPMAn":
												...
												b.pressed.connect(_show_section.bind(items[i]))
								root.add_child(b)
				root.add_child(_build_pilot_info_panel())
				var continue_button := _large_button("OYUNA DÖn")
				continue_button.position = Vector2(430, 470)
				continue_button.size = Vector2(300, 60)
				continue_button.pressed.connect(_close_all)
				root.add_child(continue_button)
				return root

func _build_pilot_info_panel() -> Control:
	var panel := PanelContainer.new()
	panel.name = "PilotInfo"
	...
	var heading := Label.new()
	heading.text = "PİLOT BİLGİLERİ"
	NovaGateUITheme.title_label(heading, 20)
	rows.add_child(heading)
	for field in ["Pilot", "Rank", "Level", "EXP", "Honor", "Company", "BTC", "PLT"]:
		var row := HBoxContainer.new()
		row.name = "PilotRow" + field
		row.add_theme_constant_override("separation", 12)
		rows.add_child(row)
		var caption := Label.new()
		caption.text = field
		caption.custom_minimum_size.x = 90
		NovaGateUITheme.dim_label(caption, 16)
		row.add_child(caption)
		if field == "Rank":
			var badge := TextureRect.new()
			badge.name = "PilotRankBadge"
			badge.custom_minimum_size = Vector2(24, 24)
			badge.expand_mode = TextureRect.EXPAND_IGNORE_SIZE
			badge.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
			row.add_child(badge)
		var value := Label.new()
		value.name = "PilotValue" + field
		value.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		value.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
		value.add_theme_font_size_override("font_size", 16)
		row.add_child(value)
	_update_pilot_info_panel(panel)
	return panel

func _update_pilot_info_panel(panel: Control) -> void:
	var values := {
		"Pilot": GlobalState.nickname if not GlobalState.nickname.is_empty() else GlobalState.username,
		"Rank": GlobalState.display_rank_title(),
		"Level": str(GlobalState.level),
		"EXP": _format_number(GlobalState.xp),
		"Honor": _format_number(GlobalState.honor),
		"Company": GlobalState.company,
		"BTC": _format_number(GlobalState.bitcoin),
		"PLT": _format_number(GlobalState.platinum)
	}
	for field in values:
		var label := panel.find_child("PilotValue" + field, true, false) as Label
		if label != null:
			label.text = str(values[field])
	var badge := panel.find_child("PilotRankBadge", true, false) as TextureRect
	if badge != null:
		badge.texture = RankBadge.texture_for(GlobalState.display_rank_key())
...
func open_skill_tree(refresh_from_server: bool = true) -> void:
	...
	skill_scroll.name = "SkillTreeContainer"
	section_content_panel.add_child(skill_scroll)
	skill_scroll.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	var skill_container := Panel.new()
	skill_container.name = "SkillTreeCanvas"
	skill_container.custom_minimum_size = Vector2(930, 520)
	...
	for idx in range(skills.size()):
		...
		match skill_id:
			"laser_power":
				icon_path = "res://assets/equipment/lf3.png"
			"npc_damage":
				icon_path = "res://market/assets/boosters/DMG-B01.png"
			"critical_damage":
				icon_path = "res://assets/equipment/lf2.png"
			"shield_power":
				icon_path = "res://assets/equipment/shield2.png"
			"hp_power":
				icon_path = "res://market/assets/boosters/HP-B01.png"
			"motor_power":
				icon_path = "res://assets/equipment/speed2.png"
...
func refresh_info():
	var info := find_child("PilotInfo", true, false) as Control
	if info != null:
		_update_pilot_info_panel(info)
# Mobil KONFİ butonu, PC C tuşuyla aynı sistemi kullanır.
...
func _open_settings_screen() -> void:
	...
	settings_root.name = "SettingsRoot"
	...
	section_content_panel.add_child(settings_root)

	var top_bar := HBoxContainer.new()
	top_bar.add_theme_constant_override("separation", 12)
	settings_root.add_child(top_bar)
	var title := Label.new()
	title.text = "AYARLAR"
	NovaGateUITheme.title_label(title, 22)
	...
	reset_button.text = "TÜM AYARLARI SIFIRLA"
	NovaGateUITheme.apply_button(reset_button)
	reset_button.pressed.connect(func() -> void:
		var confirm := ConfirmationDialog.new()
		confirm.title = "Ayarları sıfırla"
		confirm.dialog_text = "Tüm ayarlar varsayılana döndürülecek. Devam edilsin mi?"
		confirm.confirmed.connect(func() -> void:
			SettingsManager.reset_to_defaults()
			if is_instance_valid(section_content_panel):
				for child in section_content_panel.get_children():
					if child.name == "SettingsRoot":
						child.queue_free()
						break
				_open_settings_screen()
		)
	...
	settings_root.add_child(split)

	var categories_box := VBoxContainer.new()
	categories_box.custom_minimum_size = Vector2(200, 0)
	categories_box.add_theme_constant_override("separation", 8)
	split.add_child(categories_box)
	var content_box := ScrollContainer.new()
	content_box.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	content_box.size_flags_vertical = Control.SIZE_EXPAND_FILL
	content_box.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED
	split.add_child(content_box)
	var current_content := VBoxContainer.new()
	settings_content_container = current_content
	current_content.name = "SettingsContentContainer"
	current_content.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	current_content.size_flags_vertical = Control.SIZE_EXPAND_FILL
	current_content.add_theme_constant_override("separation", 10)
	content_box.add_child(current_content)
	var categories := ["HESAP", "OYUN", "GÖRÜNTÜ", "SES", "KONTROLLER", "ARAYÜZ", "MOBİL KONTROLLER", "DİL", "ERİŞİLEBİLİRLİK"]
	var selected_category := "HESAP"
	for category in categories:
		var button := Button.new()
		button.text = category
		button.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		NovaGateUITheme.apply_button(button)
		button.pressed.connect(func() -> void:
			selected_category = category
			if settings_content_container != null:
				for child in settings_content_container.get_children():
					child.queue_free()
			_generate_settings_category(settings_content_container, category)
		)
		categories_box.add_child(button)
	_generate_settings_category(settings_content_container, selected_category)

func _generate_settings_category(container: VBoxContainer, category: String) -> void:
	...
	NovaGateUITheme.title_label(heading, 18)
	...
	match category:
		"HESAP":
			var nickname_label := Label.new(); nickname_label.text = "Oyun içi nickname"; panel_box.add_child(nickname_label)
			var nickname_edit := LineEdit.new(); nickname_edit.text = str(GlobalState.nickname if not GlobalState.nickname.is_empty() else GlobalState.username); panel_box.add_child(nickname_edit)
			var cooldown_label := Label.new(); cooldown_label.text = ""; panel_box.add_child(cooldown_label)
			var current_password := LineEdit.new(); current_password.secret = true; current_password.placeholder_text = "Mevcut şifre"; panel_box.add_child(current_password)
			var new_password := LineEdit.new(); new_password.secret = true; new_password.placeholder_text = "Yeni şifre"; panel_box.add_child(new_password)
			var account_row := HBoxContainer.new(); account_row.add_theme_constant_override("separation", 8); panel_box.add_child(account_row)
			var save_button := Button.new(); save_button.text = "KAYDET"; NovaGateUITheme.apply_button(save_button); save_button.pressed.connect(func() -> void:
				var am = load("res://scripts/account_manager.gd").new()
				var nick_result = am.change_nickname(nickname_edit.text)
				if not nick_result["ok"]:
					cooldown_label.text = str(nick_result["message"])
					print("SETTINGS NICKNAME ERROR: ", nick_result["message"])
				else:
					cooldown_label.text = "Nick değiştirildi. 7 günlük cooldown aktif."
				if not current_password.text.is_empty() or not new_password.text.is_empty():
					var pass_result = am.change_password(current_password.text, new_password.text)
					print("SETTINGS PASSWORD: ", pass_result["message"])
				am.queue_free()
			)
			account_row.add_child(save_button)
			var cooldown_remaining: String = load("res://scripts/account_manager.gd").new().format_nickname_cooldown_remaining()
			if not cooldown_remaining.is_empty():
				cooldown_label.text = cooldown_remaining
				cooldown_label.modulate = Color(1.0, 0.6, 0.6, 1.0)
			var auto_login := CheckBox.new(); auto_login.text = "Otomatik giriş"; auto_login.button_pressed = bool(SettingsManager.get_setting("account", "auto_login", false)); auto_login.toggled.connect(func(value: bool) -> void:
				SettingsManager.set_setting("account", "auto_login", value)
			); panel_box.add_child(auto_login)
		"OYUN":
...
func _can_modify_hangar_state(action_name: String) -> bool:
	if player == null:
		return true
	var is_safe: bool = true
	if player.has_method("get"):
		var safe_zone_value = player.get("in_safe_zone")
		if safe_zone_value != null:
			is_safe = bool(safe_zone_value)
	if not is_safe:
		selection_label.text = "%s için güvenli bölgede olmalısın." % action_name
		return false
	if player.has_method("get"):
		var in_combat_value = player.get("is_in_combat")
		if in_combat_value != null:
			var in_combat := bool(in_combat_value)
			if in_combat:
				selection_label.text = "%s savaş sırasında yapılmaz." % action_name
				return false
	return true

func _cancel_pending_ship_change(message: String) -> void:
	ship_change_pending = false
	ship_change_target_id = ""
	ship_change_elapsed = 0.0
	if selection_label != null:
		selection_label.text = message

func _commit_pending_ship_change() -> void:
	var ship_id := ship_change_target_id
	if ship_id.is_empty():
		ship_change_pending = false
		ship_change_target_id = ""
		ship_change_elapsed = 0.0
		return
	if not _can_modify_hangar_state("Gemiyi değiştirmek"):
		ship_change_pending = false
		ship_change_target_id = ""
		ship_change_elapsed = 0.0
		return
...
func _activate_owned_ship(ship_id:String) -> void:
	if ship_change_pending:
		selection_label.text = "Gemi değişimi zaten sürüyor."
		return
	if not _can_modify_hangar_state("Gemiyi değiştirmek"):
		return
	ship_change_pending = true
	ship_change_target_id = ship_id
	ship_change_elapsed = 0.0
	if selection_label != null:
		selection_label.text = "GEMİ DEĞİŞTİRİLİYOR... 5"
...
func _process(delta: float) -> void:
	if not ship_change_pending:
		return
	if player == null:
		_cancel_pending_ship_change("Gemi değişimi iptal edildi.")
		return
	var is_safe: bool = true
	if player != null and player.has_method("get"):
		...
		if safe_zone_value != null:
			is_safe = bool(safe_zone_value)
	if not is_safe:
		_cancel_pending_ship_change("GEMİ DEĞİŞTİRMEK İÇİN GÜVENLİ BÖLGEDE OLMALISIN.")
		return
	ship_change_elapsed += delta
	var remaining_seconds: int = int(maxi(0.0, ship_change_total_seconds - ship_change_elapsed))
	if selection_label != null:
		selection_label.text = "GEMİ DEĞİŞTİRİLİYOR... %d" % remaining_seconds
	if ship_change_elapsed >= ship_change_total_seconds:
		_commit_pending_ship_change()
...
func equip_purchased_item(item_name: String) -> void:
	# Marketten alınan ekipman MenuUI'ye anında aktarılır.
	# Önce players.json'daki güncel envanteri al, sonra aktif geminin
	# aktif konfigürasyonunda ilk uygun yuvaya yerleştir ve kaydet.
	_load_ship_catalog()
	var account_manager = load("res://scripts/account_manager.gd").new()
	var username: String = account_manager.get_current_player()
	var saved_player = account_manager.get_player(username)
	...
	# LF1/LF2/LF3 ve diğer gemi ekipmanları gemi sekmesine takılır.
	# Böylece markette hangi sekmenin açık olduğundan etkilenmez.
	selected_tab = "ship"
	selected_item = ""

	if _auto_equip_selected(item_name):
		selection_label.text = "%s satın alındı ve aktif Konfigürasyon %d'ye takıldı." % [str(ITEM_DATA.get(item_name, {}).get("title", item_name)), selected_config]
	else:
