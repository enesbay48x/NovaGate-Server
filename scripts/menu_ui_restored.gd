...
var config_one_button: Butto

var config_two_button: Butto

var ship_tab_button: Butto

var drone_tab_button: Butto
...
# A rutbesi panelinde ayni anda listelenen oyuncu sayisi.nconst A_RANK_ADMIn_ROWS := 6nconst RankData = preload("res://scripts/rank_data.gd")nconst RankService = preload("res://scripts/rank_service.gd")nconst RankBadge = preload("res://scripts/rank_badge.gd")nconst SHIP_LASER_SLOTS := 60nconst SHIP_GENERATOR_SLOTS := 32nconst NORMAL_LASER_SLOT_CAP := 30nconst NORMAL_GENERATOR_SLOT_CAP := 16nconst SHIP_EXTRA_SLOTS := 8nconst DRONE_COUNT := 8nconst DRONE_SLOTS_PER_DRONE := 2nconst ITEM_DATA: Dictionary = {
				...
				"HOn-B01": {"icon": "res://market/assets/boosters/HOn-B01.png", "type": "extra", "title": "HOn-B01"}n}
...
var migrate_local_loadout_to_server: bool = falsenfunc _ready() -> void:
				add_to_group("menu_ui")
				QuestSystem.quests_changed.co
nect(_refresh_available_quests, CO
NECT_DEFERRED)
	...
	if migrate_local_loadout_to_server:
		call_deferred("_queue_server_loadout_sync")nfunc _normalize_configuration(config_value) -> Dictionary:
	...
	_refresh_all()nfunc _owned_droid_slot_indices() -> Array:
	var result: Array = []
	...
	return resultnfunc _new_configuration() -> Dictionary:
				var lasers: Array = []
				lasers.resize(SHIP_LASER_SLOTS)
