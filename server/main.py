...
async def register(request: Request, req: RegisterRequest):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # unique player_id. Starting balance is zero; nothing is copied
        # from any other account.
        await db.execute(
            "INSERT OR IGNORE INTO economy (player_id, btc, plt, gold, updated_at) VALUES (?, 0, 0, 0, ?)",
            (player_id, now),
        )
        await db.commit()
...
async def login(request: Request, req: LoginRequest):
    ...
    return {
        ...
        # ship_configurations, droid_types) are intentionally OMITTED rather
        # than sent as empty defaults. Sending empty defaults made the client
        # overwrite a real player's ships/loadout with "Ship10 only" on every
        # login, which silently destroyed progression.
        "oyuncu": {
            "id": player_id,
            "username": username,
            "company": company,
