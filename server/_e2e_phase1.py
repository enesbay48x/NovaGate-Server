"""Phase 1 - REAL Godot client E2E harness.

Starts a real NovaGate server on a test database, then runs the REAL Godot
4.7.2 client driver (scripts/p1_e2e_driver.gd) against it and asserts on the
machine-readable P1_E2E_RESULT line it prints.

The driver uses the project's own scripts (account_manager.gd,
novagate_ws_client.gd, GlobalState.gd) - nothing is mocked.

Usage:  python _e2e_phase1.py
...
SECRET = "phase1-e2e-secret-key-0123456789abcdefghij"
DRIVER = "res://scenes/p1_e2e.tscn"
...
def start_server(log_path: str):
    db_path = os.path.join(HERE, "test_novagate_p1_e2e.db")
...
def run_godot_driver(log_path: str, timeout: float = 240.0) -> str:
    godot = find_godot()
    # A per-run user:// dir so a previous run's players.json / _save.json can
    # never make this run look successful.
    user_dir = tempfile.mkdtemp(prefix="p1e2e_user_")
    env = dict(os.environ)
...
def parse_result(log_path: str):
    ...
    for line in text.splitlines():
        if line.startswith("P1_E2E_RESULT "):
            return json.loads(line[len("P1_E2E_RESULT "):])
    return None


def main() -> int:
    server_log = os.path.join(HERE, "_e2e_server.log")
    godot_log = os.path.join(HERE, "_e2e_godot.log")
    ...
    try:
        ...
        print("\n--- godot output (tail) ---")
        print("\n".join(godot_text.splitlines()[-60:]))

        if result is None:
            print("\nFAIL: driver produced no P1_E2E_RESULT line (rc=%s)" % rc)
            return 1

        print("\n--- Phase 1 real-client E2E checks ---")
        ...
        if rc not in (0, None):
            ...
            return 1
        print("\nPHASE1_REAL_GODOT_E2E=PASS  user=%s" % result["username"])
        return 0
    finally:
        proc.terminate()
        try:
